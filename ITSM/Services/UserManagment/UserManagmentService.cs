    ...
    */
    ...
    public async Task EditUser(string id, EditUserViewModel editmodel)
    {
        ...
        if (!string.IsNullOrEmpty(editmodel.NewPassword) && !string.IsNullOrEmpty(editmodel.ConfirmPassword))
        {
            ...
            {
                throw new ArgumentException("Новй пароль и підтвердження пароля не співпадають.");
            }
            ...
            if (removePasswordResult.Succeeded)
            {
                ...
                if (!addPasswordResult.Succeeded)
                {
                    ...
                    throw new Exception($"Не вдалось змінити пароль: {errors}");
                }
            }
            else
            {
                ...
                throw new Exception($"Не вдалось видалити старий пароль: {errors}");
            }
        }
        await dBaseContext.SaveChangesAsync();
    }
    ...
    {
        var user = await GetUserById(userId) ?? throw new ArgumentException("Користувач не найден");
